...
public class VersionService : IVersionService
{
  ...
  {
    _fileService.Value.CheckIfFileExists( fileId );
  ...
  {
    MediaFile? file = _fileService.Value.CheckIfFileExists( data.FileId );

    CheckByPathIfFileExists( file.FilePath );

    User? user = _dbContext.Users.FirstOrDefault( user => user.Id == file.UserId );

    _userService.Value.CheckIfUserIsNull( user );
  ...
  public FileVersionDto RestoreVersion( int versionId )
  {
    ...
    _fileService.Value.CheckIfFileIsNull( file );
  ...
  public void CheckByPathIfFileExists( string path )
  {
    if ( !File.Exists( path ) )
    {
